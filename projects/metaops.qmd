...
Rather than letting users create resources without limit, I designed and implemented a structure that manages cloud resources through a request-and-approval flow.

```{mermaid}
flowchart TD
    U[User] --> RQ[Resource Request]
    RQ --> AR[Admin Review]
    AR --> AP[Approval]
    AP --> PR[OpenStack Resource Provisioning]
```

- Developed the UI and API for resource requests/approvals
- Built automated resource request/approval workflows using **Adjutant** and **Mistral**
- Automated OpenStack resource provisioning for approved requests
